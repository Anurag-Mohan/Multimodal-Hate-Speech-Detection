...
print(f"🔥 V10: Custom CNN + BiLSTM + GloVe Knowledge on: {device}")
...
def download_glove():
    if not os.path.exists('glove.6B.100d.txt'):
        print("--- Downloading GloVe (World Knowledge)... This takes a minute ---")
...
def load_glove_vectors(vocab):
    ...
    print(f"✅ Injected Knowledge for {hits} words out of {len(vocab)}.")
...
class TextBiLSTM_GloVe(nn.Module):
    def __init__(self, vocab_size, embedding_matrix):
        super(TextBiLSTM_GloVe, self).__init__()
...
class MultimodalNet(nn.Module):
    def __init__(self, vocab_size, embedding_matrix):
        ...
        self.cnn = CustomCNN()
        self.lstm = TextBiLSTM_GloVe(vocab_size, embedding_matrix)
...
def train_pipeline():
    ...
    embedding_matrix = load_glove_vectors(vocab)
    ...
    model = MultimodalNet(len(vocab), embedding_matrix).to(device)
    
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    criterion = nn.BCELoss()

    print("--- PHASE 1: Training Knowledge-Enhanced Extractors ---")
    ...
    print("\n--- PHASE 2: Applying SMOTE to Knowledge Features ---")
    model.eval()
    all_features = []
    all_labels = []
    with torch.no_grad():
    ...
    print(f"Original Shape: {X_train.shape} | Balancing...")
    smote = SMOTE(random_state=42)
    X_res, y_res = smote.fit_resample(X_train, y_train)
    print(f"New Knowledge-SMOTE Shape: {X_res.shape}")
    ...
    print("--- Retraining Classifier Head ---")
    best_acc = 0.0
    for epoch in range(EPOCHS_PHASE_2):
        ...
        print(f"Phase 2 Epoch {epoch+1} | Val Acc: {acc:.2f}%")
        
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), 'meme_model_v10_glove.pth')
            import pickle
            with open('vocab_v10.pkl', 'wb') as f: pickle.dump(vocab, f)

    print(f"\n✅ Final Accuracy (Custom + GloVe + SMOTE): {best_acc:.2f}%")
