...
print(f"🔥 Training Custom ResNet + Co-Attention on: {device}")
...
class CustomResNet(nn.Module):
    def __init__(self):
        ...
        self.fc_proj = nn.Linear(256, 128) 
...
def train_model():
    ...
    print(f"--- Starting V5 (ResNet + Co-Attention) Training ---")
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1}/{EPOCHS} | Val Acc: {epoch_val_acc:.2f}% | LR: {optimizer.param_groups[0]['lr']:.6f}")
    ...
    torch.save(best_model_wts, 'meme_model_v5.pth')
    import pickle
    with open('vocab_v5.pkl', 'wb') as f:
        pickle.dump(vocab, f)
